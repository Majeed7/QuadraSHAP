"""
LogisticExplainer: QuadraSHAP for logistic regression on the odds scale, start to finish, for ONE instance.

Flat script, no functions: set the knobs at the top, run it, step through it in a
debugger.  It fits a scikit-learn LogisticRegression to synthetic binary data, chooses the number of Gauss--Legendre nodes a priori
...
efficiency identity.  Attributions are on the odds P(y=1)/P(y=0) scale.
    ...
    python run_logistic_example.py
"""
import itertools
import math
import sys
...
from quadrashap import LogisticExplainer
...
beta_true = np.zeros(D); beta_true[:4] = [1.2, -0.8, 0.5, 0.3]
y = (rng.random(len(X)) < 1 / (1 + np.exp(-(0.4 + X @ beta_true)))).astype(int)
X_train, y_train, X_test = X[:N_TRAIN], y[:N_TRAIN], X[N_TRAIN:]
t0 = time.perf_counter()
model = LogisticRegression(C=C).fit(X_train, y_train)
print(f"trained LogisticRegression on {N_TRAIN} x {D} in {time.perf_counter()-t0:.2f}s")
...
# ----------------------------------------------------------------------------- explainer
explainer = LogisticExplainer(model, background=background, eps=EPS, backend=BACKEND)
...
print(f"log-odds coefficients beta = {explainer.beta}, intercept b0 = {explainer.log_intercept:.4f}")
print(f"P(y=1|x) = {model.predict_proba(x[None, :])[0, 1]:.4f}, odds = {F(x[None, :])[0]:.4f}   (attributions are on the odds scale)")
