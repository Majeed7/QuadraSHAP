"""
PoissonExplainer: QuadraSHAP for Poisson regression with a log link, start to finish, for ONE instance.

Flat script, no functions: set the knobs at the top, run it, step through it in a
debugger.  It fits a scikit-learn PoissonRegressor to synthetic count data, chooses the number of Gauss--Legendre nodes a priori
...
efficiency identity.  Attributions are on the expected-count (mean) scale.
...
from quadrashap import PoissonExplainer
...
TEST_INDEX = 0
RIDGE = 1e-3            # PoissonRegressor(alpha=...)
...
model = PoissonRegressor(alpha=RIDGE).fit(X_train, y_train)
print(f"trained PoissonRegressor on {N_TRAIN} x {D} in {time.perf_counter()-t0:.2f}s")

# F(x): the multiplicative-scale quantity that is attributed (expected-count (mean))
...
# ----------------------------------------------------------------------------- explainer
explainer = PoissonExplainer(model, background=background, eps=EPS, backend=BACKEND)
...
print(f"coefficients beta = {explainer.beta}, intercept b0 = {explainer.log_intercept:.4f} (component coefficient e^b0 = {explainer.coef[0]:.4f})")
