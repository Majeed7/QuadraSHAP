"""
NaiveBayesExplainer: QuadraSHAP for naive Bayes on the odds scale, start to finish, for ONE instance.

Flat script, no functions: set the knobs at the top, run it, step through it in a
debugger.  It fits a scikit-learn GaussianNB to synthetic binary data (swap in BernoulliNB / MultinomialNB with the knob), chooses the number of Gauss--Legendre nodes a priori
...
efficiency identity.  Attributions are on the odds P(y=1)/P(y=0) scale.

    python run_naive_bayes_example.py
"""
import itertools
import math
import sys
...
from quadrashap import NaiveBayesExplainer
...
TEST_INDEX = 0
NB_KIND = "gaussian"    # "gaussian", "bernoulli" or "multinomial"

# ----------------------------------------------------------------------------- data + model
rng = np.random.default_rng(SEED)
X = rng.standard_normal((N_TRAIN + 10, D)) * 0.7
y = (rng.random(len(X)) < 1 / (1 + np.exp(-(0.4 + 1.2 * X[:, 0] - 0.8 * X[:, 1] + 0.5 * X[:, 2])))).astype(int)
if NB_KIND == "bernoulli":
    X = (X > 0).astype(float)                       # binary indicators
elif NB_KIND == "multinomial":
    X = rng.poisson(np.exp(0.5 * X)).astype(float)  # non-negative counts
X_train, y_train, X_test = X[:N_TRAIN], y[:N_TRAIN], X[N_TRAIN:]
t0 = time.perf_counter()
model = {"gaussian": GaussianNB, "bernoulli": BernoulliNB, "multinomial": MultinomialNB}[NB_KIND]().fit(X_train, y_train)
print(f"trained naive Bayes ({NB_KIND}) on {N_TRAIN} x {D} in {time.perf_counter()-t0:.2f}s")

# F(x): the multiplicative-scale quantity that is attributed (odds P(y=1)/P(y=0))
F = lambda P: model.predict_proba(P)[:, 1] / model.predict_proba(P)[:, 0]    # odds of class 1 vs class 0
...
# ----------------------------------------------------------------------------- explainer
explainer = NaiveBayesExplainer(model, background=background, eps=EPS, backend=BACKEND)
...
print(f"prior odds P(1)/P(0) = {explainer.coef[0]:.4f};  P(y=1|x) = {model.predict_proba(x[None, :])[0, 1]:.4f}, odds = {F(x[None, :])[0]:.4f}")
