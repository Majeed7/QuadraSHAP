"""
GammaExplainer: QuadraSHAP for Gamma regression with a log link, start to finish, for ONE instance.

Flat script, no functions: set the knobs at the top, run it, step through it in a
debugger.  It fits a scikit-learn GammaRegressor to synthetic positive data, chooses the number of Gauss--Legendre nodes a priori
...
from quadrashap import GammaExplainer
...
TEST_INDEX = 0
RIDGE = 1e-3            # GammaRegressor(alpha=...)

# ----------------------------------------------------------------------------- data + model
rng = np.random.default_rng(SEED)
X = rng.standard_normal((N_TRAIN + 10, D)) * 0.7
beta_true = np.zeros(D); beta_true[:3] = [0.6, -0.4, 0.3]
y = rng.gamma(shape=2.0, scale=np.exp(0.2 + X @ beta_true) / 2.0)
X_train, y_train, X_test = X[:N_TRAIN], y[:N_TRAIN], X[N_TRAIN:]
t0 = time.perf_counter()
model = GammaRegressor(alpha=RIDGE).fit(X_train, y_train)
print(f"trained GammaRegressor on {N_TRAIN} x {D} in {time.perf_counter()-t0:.2f}s")
...
# ----------------------------------------------------------------------------- explainer
explainer = GammaExplainer(model, background=background, eps=EPS, backend=BACKEND)
