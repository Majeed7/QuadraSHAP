"""
GLMExplainer: QuadraSHAP for any log-link GLM given by its coefficients (e.g. from statsmodels), start to finish, for ONE instance.

Flat script, no functions: set the knobs at the top, run it, step through it in a
debugger.  It fits a Poisson GLM with a log link by a few Newton (IRLS) steps written out in NumPy, so no modelling library is needed, and hands the coefficients to GLMExplainer(beta, intercept), chooses the number of Gauss--Legendre nodes a priori
...
efficiency identity.  Attributions are on the mean-response scale.

    python run_glm_example.py
"""
import itertools
import math
import sys
...
from quadrashap import GLMExplainer
...
beta_true = np.zeros(D); beta_true[:4] = [1.0, -0.5, 0.3, 0.2]
y = rng.poisson(np.exp(0.3 + X @ beta_true))
X_train, y_train, X_test = X[:N_TRAIN], y[:N_TRAIN], X[N_TRAIN:]
t0 = time.perf_counter()
A = np.column_stack([np.ones(N_TRAIN), X_train])       # design matrix with intercept column
theta = np.zeros(D + 1)
for _ in range(25):                                     # IRLS for Poisson with log link
    mu = np.exp(A @ theta)
    theta += np.linalg.solve(A.T @ (mu[:, None] * A) + 1e-6 * np.eye(D + 1), A.T @ (y_train - mu))
intercept, beta = theta[0], theta[1:]
print(f"trained Poisson GLM (IRLS in NumPy) on {N_TRAIN} x {D} in {time.perf_counter()-t0:.2f}s")

# F(x): the multiplicative-scale quantity that is attributed (mean-response)
F = lambda P: np.exp(intercept + np.atleast_2d(P) @ beta)     # mu(x) = exp(b0 + beta.x)
...
# ----------------------------------------------------------------------------- explainer
explainer = GLMExplainer(beta, intercept, background=background, eps=EPS, backend=BACKEND)
...
print(f"coefficients beta = {beta}, intercept b0 = {intercept:.4f}")
